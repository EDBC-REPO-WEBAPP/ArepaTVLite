...
page.similar = async function(x){
    ...
    const {data} = await fetch(`http://localhost:27017/match${params}`); return data.map((y)=>{ return `
        <a onclick="show(this)" hash="${y.hash}" type="${x.type}" id="movie" class="uk-inline uk-child-width-expand uk-height-medium">
            <img class="uk-rounded" src="./img/placeholder.webp" lazy="${y.imagen}" alt="${y.nombre}">
            <div class=" uk-position-top-left uk-position-small">
                <span class="uk-badge uk-primary uk-enphasis"> ${x.type} </span>   
            </div>  
        </a>
    `;}).join('');
}
...
page.capitulos = function(x){
    ...
    x.magnet.map((temporada,i)=>{
        ...
        const cap = temporada.map((capitulo,j)=>{ return `
            <a  target="_blank" id="movie" onclick="play(this)" class="uk-inline uk-child-width-expand uk-height-small"
                url="/app/play?hash=${x.hash}&cap=${j}&tmp=${i}&type=${x.type}" >             
                <img class="uk-rounded" src="./img/thumbnail.webp" lazy="${capitulo.imagen}" alt="">
                <div class=" uk-position-top-left uk-position-small">
...
page.screen = async function(x){
    ...
    if( typeof x.magnet == 'string' ) return `
...
module.exports = async(req,res)=>{
    ...
    fetch(`http://localhost:27017/hash${params}`).then(async({data})=>{
